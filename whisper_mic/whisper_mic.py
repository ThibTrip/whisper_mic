...
class WhisperMic:
    def __init__(self,model="base",device=("cuda" if torch.cuda.is_available() else "cpu"),english=False,verbose=False,energy=300,pause=2,dynamic_energy=False,
                 save_file=False, model_root="~/.cache/whisper",mic_index=None,implementation="whisper",hallucinate_threshold=300,
                 task='transcribe'):
    ...
    def __transcribe(self,data=None, realtime: bool = False) -> None:
        ...
        if is_audio_loud_enough:
            ...
            if self.faster:
                segments, info = self.audio_model.transcribe(audio_data, task=self.task)
                for segment in segments:
                    predicted_text += segment.text
            else:
                if self.english:
                    result = self.audio_model.transcribe(audio_data, task=self.task,
                                                         language='english',suppress_tokens="")
                else:
                    result = self.audio_model.transcribe(audio_data, task=self.task,
                                                         suppress_tokens="")
